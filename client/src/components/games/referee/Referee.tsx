...
export const getCaptureFunction = (
  pieceType: PieceType,
  selectedPiece: Piece,
  pieces: Piece[]
): { x: number; y: number }[] => {
...
export const getMovesFunctionKing = (
  pieceType: PieceType,
  selectedPiece: Piece,
  pieces: Piece[],
  currentTurn: String
  ...
  switch (pieceType) {
    ...
    case PieceType.KING:
      return getPossibleKingMovesKing(selectedPiece, pieces);
...
export const handleThreefoldRepetition = (move: string[]): boolean => {
    const length = move.length;
    
    if (length > 7) {
      return (
        move[length - 1] === move[length - 5] &&
        move[length - 2] === move[length - 6] &&
        move[length - 3] === move[length - 7] &&
        move[length - 4] === move[length - 8]
      );
    }
  ...
  export const insufficientMaterial = (pieces: Piece[]): boolean => {
  if (pieces.length === 2) {
    return true;
  }
...
export const isStalemate = (
  ...
  for (const piece of sameColorPieces) {
    ...
    const possibleMoves = getMovesFunction(piece.type, piece, updatedPieces, null);

    for (const move of possibleMoves) {
      // Simular cada movimiento y verificar si deja al rey en jaque
      const isCheckAfterMove = isSimulatedMoveCausingCheck(piece, move.x, move.y, updatedPieces, color);
  ...
  for (const move of kingMoves) {
    const isCheckAfterMove = isSimulatedMoveCausingCheck(king1, move.x, move.y, piecesCopy, color);
